...
public class EmployeeService(IUnitOfWork unitOfWork) : IEmployeeService
{
    public async Task<PagedResult<EmployeeDto>> GetAllEmployeesAsync(int companyId, int pageNumber, int pageSize, CancellationToken ct = default)
    {
        var employees = await unitOfWork.Employees
            .GetAllEmployeesByCompanyAsync(companyId, pageNumber, pageSize, ct);
        return employees.MapTo<Employee, EmployeeDto>();
    }
    ...
    public async Task<EmployeeDto> CreateEmployeeAsync(CreateEmployeeDto request, int companyId, int userId, CancellationToken ct = default)
    {
        var employee = request.Project();
    ...
    public async Task<EmployeeDto> UpdateEmployeeAsync(int id, int companyId, UpdateEmployeeDto request, int userId, CancellationToken ct = default)
    {
        var employee = await unitOfWork.Employees
            .GetEmployeeByIdAsync(id, companyId, ct);
